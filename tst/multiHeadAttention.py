...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self,
                ...
                key: torch.half,
                value: torch.half,
                mask: Optional[str] = None) -> torch.half:
    ...
    @property
    def attention_map(self) -> torch.half:
...
class MultiHeadAttentionChunk(MultiHeadAttention):
    ...
    attention_size:
        ...
        Deactivated if ``None``. Default is ``None``.
    chunk_size:
        Size of chunks to apply attention on. Last one may be smaller (see :class:`torch.half.chunk`).
        Default is 168.
    """
