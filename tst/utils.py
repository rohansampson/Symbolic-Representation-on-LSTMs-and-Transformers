...
def generate_original_PE(length: int, d_model: int) -> torch.half:
    """Generate positional encoding as described in original paper.  :class:`torch.half`
...
def generate_regular_PE(length: int, d_model: int, period: Optional[int] = 24) -> torch.half:
